...
### STEP 1. 크롤링 구현
1. 타겟 URL 1개 또는 URL 목록 파일 선정
2. (선택) 검색어 기반 URL 자동 수집으로 입력 대상 생성
3. 페이지 접근/본문 추출
4. 리뷰 최소 1건 이상 확보
5. 샘플 출력으로 수집 성공 검증
6. 다중 URL 처리 시 카페별 성공/실패 통계 기록
...
## 8) 현재 상태 스냅샷 (2026-05-02)

- STEP1: 네이버 블로그 URL 본문 수집 성공 (모바일 URL 정규화 반영)
- STEP2: MongoDB 저장 성공
- STEP3: 광고 필터 키워드 확장 완료
- STEP4: 메뉴 추출 사전/별칭 정규화 적용
- STEP5: MySQL 스키마 초기화 + upsert 적재 성공
- 검증 결과: 특정 게시글은 메뉴 언급 부족으로 빈도 0이 정상이며, 메뉴 포함 게시글은 MySQL 반영 확인

---

## 9) 다음 세부 실행 계획

1. `--urls-file` 기반 다중 URL 수집 파이프라인 추가
2. 카페명/소스별 수집 성능 로그 저장
3. `menu_candidate` 자동 적재(사전 미매칭 용어)
4. API 응답을 Mongo 실시간 분석 + MySQL 집계 fallback 구조로 개선
5. Render 배포 및 운영 체크리스트 문서화
